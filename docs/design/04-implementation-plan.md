...
- **WebCodecs compositor slicing (started 2026-07-09):**
  ...
  - **PR 24 — audio mixing:** AudioContext graph (decodeAudioData on proxies), per-clip gain, audio tracks, audio-clock-driven sync.
- **Remaining M5 arcs (after compositor):** ripple trim, take-aware shot-clip source binding (image takes, blade offsets). Backlog: transactional `ReorderShots(scene_id, ordered_ids)` RPC — the board renumbers client-side today (N sequential UpdateShot calls, non-atomic on partial failure, self-healing on the next reorder).
