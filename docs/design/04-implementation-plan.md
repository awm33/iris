...
- **Remaining M5 arcs:** WebCodecs compositor + multi-track/gapless playback (incl. audio mixing), ripple trim, take-aware shot-clip source binding (image takes, blade offsets). Backlog: transactional `ReorderShots(scene_id, ordered_ids)` RPC — the board renumbers client-side today (N sequential UpdateShot calls, non-atomic on partial failure, self-healing on the next reorder).
