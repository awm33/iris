...
- **PR 35 — transcribe → caption track** (design locked 2026-07-10): captions are TIMELINE OPS, not a side table — new track kind `caption`, `text` on ClipInit, and a `set_clip_text` op, in BOTH reducers + a shared vector (the op-log stays the single document of record; export burns in from the same reduced state it renders). Transcription = media job kind `transcribe`: reuse the export pipeline's reduce/resolve/download steps to render the timeline's mixed audio (audio-only ffmpeg graph → 16k mono wav), run the engine, then APPEND ops (add caption track if absent + one text clip per segment) via AppendTimelineOps with CAS retry. Engine seam: `IRIS_WHISPER_BIN` — any whisper.cpp-CLI-compatible binary (`-oj` JSON with per-segment offsets); dev uses `backend/dev/mock-whisper` (deterministic: ffprobe duration → one placeholder segment per ~2.5s) since no host whisper exists and real local ASR is a provisioning item before the dogfood; unset → job parks with a clear config error. `transcriptions` table + Start/ListTranscriptions RPCs mirror exports' lifecycle row. UI: caption clips render as text chips on their track (double-click edits → set_clip_text), active caption overlays the preview (both engine and <video> modes via clipAt kind=caption), 📝 Transcribe button with exports-style polling. Export: caption clips → SRT temp file → `subtitles=` burn-in appended after concat when captions exist, and the SRT stored as a derived blob (meta srt_key) on the export version.
