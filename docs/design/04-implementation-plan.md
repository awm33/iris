...
**M4 slicing (started 2026-07-08):** slice 1 ships as two PRs —
- **PR 8 — canvas foundation:** doc runtime v1 (op-log, client op-ids, undo-as-op, autosave batching; `doc_ops` + server-assigned seq via canvas head lock), CanvasService, canvas page (pan/zoom viewport, layers panel, brush/eraser on paint layers, open-image-as-canvas base layer, flatten-export to Library with lineage). Compositing starts Canvas2D behind a compositor interface — WebGL2 slots in when blend modes/adjustments demand it (per rule 4: profiling, not ideology).
- **PR 9 — gen-fill loop:** selection (marquee/lasso) → generate panel targets the canvas selection → candidate strip in place → commit as masked layer with provenance; promote-canvas-to-View. Gen-fill results are content-addressed asset versions referenced by layer ops — no tile store needed for slice 1 (tiles/snapshots when docs get heavy).
- Cut from slice 1 (per risk table): text layer v0, AI subject select (SAM/ONNX) — both slot into slice 2.
