...
- **PR 29 — adapter seam + seedance + vault:** adapters package (one client surface for registry + orchestrator; kind routes), vault (auth_ref → secret, resolved ONLY at client construction; env: refs in dev, KMS later), seedance adapter (recorded Ark v3 shapes → our spec: static manifest, text-command params, first_frame image role, result-URL download → presigned re-upload proxy path, error taxonomy incl. safety_blocked), mock-seedance server + dev endpoint seed. In-process adapters presign INTERNAL blob URLs. NOTE: recorded shapes need verification against the live API before real keys. TICKETED before real keys: persist the adapter's remote task id on the job row and re-attach on reclaim instead of re-submitting (restart/retry currently re-submits = duplicate paid generation, old task never canceled) — for SYNCHRONOUS adapters (elevenlabs) there is no remote id: re-land from the prior attempt's uploads/gen key or an idempotency marker on the job row; strip `--` flags from user prompts (text-command injection/cost control); per-workspace vault rows replace process-env refs; map ElevenLabs voice-404 → invalid_input when real voice refs land (W4). Lease note: sync CreateJob worst case (120s tts + 120s put) fits the 5m lease with 60s margin — in-adapter retries must not eat it.
- **PR 30 — elevenlabs adapter ✅ (audio modality):** synchronous TTS translator (create = generate + custody + terminal), xi-api-key auth, taxonomy mapping, usd_per_job metering; audio artifacts land kind=audio/audio-mpeg; manifest schema relaxed (profile dims 0 = not applicable for audio; 1–63 still rejected); mock-elevenlabs + tests. Default voice const — per-character voice refs land with W4.
- **PR 31 — voice params + audio refs ✅ (W4 plumbing):** params_schema-driven enum selects in the panel (voice_id first consumer; adapter routes the voice, unknown voices park invalid_input); seedance declares references.audio (spec role speech_lipsync → their lip_sync wire shape); panel audio-refs section + picker. M6 EXIT PATH VERIFIED: mara-voiced TTS → audio asset → seedance i2v with the speech_lipsync ref → landed artifact.
- **PR 32 — lipsync_post + character voice binding ✅:** conditioning.source_video wired end-to-end (API/store/orchestrator video/*-constrained resolution + lineage + cache key + pinning); lipsync_post task promptless, requires source_video; mock video declares + fetches it; panel source-clip picker; Character.voice_id (migration 0009) with CharactersPage binding input and cast→voice prefill in the panel (fills only an UNSET voice the model's enum accepts). Post-hoc fallback verified: promptless lipsync_post re-timing a video to the mara voice line → landed artifact.
- **M6 COMPLETE** (chain regen + pins, gen cache, adapter seam + vault, seedance, elevenlabs, voice params + audio refs, lipsync_post + voice binding — every path against recorded shapes, zero real-remote traffic). Next: M7 (export service v1, transcribe→captions, color basics, transitions, ducking, golden-frame CI — the full-dogfood exit). Backlog: transactional `ReorderShots(scene_id, ordered_ids)` RPC — the board renumbers client-side today (N sequential UpdateShot calls, non-atomic on partial failure, self-healing on the next reorder).

**UX affordances interlude — PR 33 ✅ (2026-07-09):** ⌘K command palette (nav, actions, entity jump over cached lists), timeline history panel over the op log (activity from the reducer's own `activeOps`; "↩ undo after" = `doc.undoTo`, redoable), `?` shortcut overlay, collapsible icon rail. Per the UX-deep-dive decision: cheap affordances now, deep-research audit in background, full design sprint after the M7 dogfood retro. Canvas history panel joins when the doc shell generalizes. Audit landed → `docs/design/05-ux-interaction-audit.md` (verified: keep fixed rail + Context Dock over Adobe-style free docking — Figma shipped floating panels in UI3 beta and reverted; contextual task bars supplement stable chrome; generative actions belong at the point of selection; op-grouping should break on navigation/selection-change, not timers; history must never destructively overwrite generative outputs. ⌘K/shortcut-overlay/FCP claims did NOT survive verification — don't cite the audit for those).

**M7 slicing (started 2026-07-09):**
- **PR 34 — export service v1 ✅:** media-worker job kind `export` (same queue/lease/ffmpeg posture as probe/prep). Go timeline reducer over the persisted op log (port of `reduceTimeline` — shared JSON test vectors guard TS/Go drift), server-side source resolution (shot clips → selected-take artifact, media clips → pinned version; image takes loop as stills; gaps render black/silence), ffmpeg filter-graph composer with preview cut semantics (topmost-track-wins, embedded + track audio mixed), `draft`/`master` H.264+AAC presets, `exports` table + Start/ListExports RPCs, result lands as a library asset (reuses SignDownload/thumbs), TimelinePage Export button + status/download. Captions burn-in point reserved in the preset (captions don't exist until the transcribe slice); transitions/color/ducking explicitly NOT in v1 — export must match what the preview plays today (cuts only). Review hardening: cuts quantize to the OUTPUT frame grid (trim=end_frame — seconds-based cuts render ceil(D·fps) frames and desync unboundedly with cut count); video seeks clamp to one frame before probed source EOF (a past-EOF seek silently drops the piece at exit 0); exports-row state agrees with the queue's retry policy (queue.WillRetry); output streams to blob; download is a pre-signed anchor (window.open-after-await is popup-blocked). Recorded deviations: busy gates on the latest export only (no concurrent draft+master), invisible unresolved shots still fail the render loudly, 15m ceiling bounds timeline length, sources re-download per retry.
- **PR 35 — transcribe → caption track ✅** (2026-07-10): captions are TIMELINE OPS, not a side table — new track kind `caption`, `text` on ClipInit, and a `set_clip_text` op, in BOTH reducers + a shared vector (the op-log stays the single document of record; export burns in from the same reduced state it renders). Transcription = media job kind `transcribe`: reuse the export pipeline's reduce/resolve/download steps to render the timeline's mixed audio (audio-only ffmpeg graph → 16k mono wav), run the engine, then APPEND ops (add caption track if absent + one text clip per segment) via AppendTimelineOps with CAS retry. Engine seam: `IRIS_WHISPER_BIN` — any whisper.cpp-CLI-compatible binary (`-oj` JSON with per-segment offsets); dev uses `backend/dev/mock-whisper` (deterministic: ffprobe duration → one placeholder segment per ~2.5s) since no host whisper exists and real local ASR is a provisioning item before the dogfood; unset → job parks with a clear config error. `transcriptions` table + Start/ListTranscriptions RPCs mirror exports' lifecycle row. UI: caption clips render as text chips on their track (double-click edits → set_clip_text), active caption overlays the preview (both engine and <video> modes via clipAt kind=caption), 📝 Transcribe button with exports-style polling. Export burn-in: Go-rendered caption PNGs (bundled Go face — the host/slim ffmpeg has NO libass/libfreetype, found live: `subtitles`/`drawtext` don't exist; overlay is universal and keeps user text out of the filter graph) with half-open gte*lt enable windows; SRT sidecar stored as derived blob (meta srt_key — not yet exposed via signDownload). Review hardening: append+complete are ONE tx with DETERMINISTIC op/clip/track ids (transcription-id-keyed — at-least-once redelivery reduces to a no-op) and CAS conflicts rebuild ops from re-read state (concurrent caption track reused, never duplicated); post-transcription reload gates on pendingCount==0 (flush resolves even on failure); bladeOps carries text. Recorded deviations: doc pickup via session reload (live remote-ops channel is the standing sync follow-up), fixed caption styling, transcribing twice stacks a second caption set (explicit user action; "replace captions" option later), per-caption overlay cost grows linearly (ceiling needed for feature-length).
- **PR 36 — color basics ✅** (2026-07-10): per-clip exposure/contrast/temp via ONE op (`set_clip_color` {clip_id, color?} — full replace, absent clears), `color` on ClipInit/ClipState in BOTH reducers + shared vector; bladeOps carries color like text. Parity-exact math chosen so preview and export share the SAME per-channel sRGB pipeline (golden-frame slice will diff them): exposure = ×2^stops (canvas `ctx.filter brightness()` ↔ ffmpeg `lutrgb val*E`), contrast = (v−0.5)·c+0.5 (canvas `contrast()` ↔ lutrgb around 127.5), temp = ATTENUATION-ONLY per-channel gains (warm cools blue/green, cool cools red — all gains ≤1 so the canvas multiply-blend rect and lutrgb agree exactly; gains >1 are impossible under multiply). Export inserts one numeric-only `lutrgb` per colored piece after the scale chain. v1 scope: video clips in ENGINE preview mode (image stills + <video> fallback follow with CSS filter/mix-blend later — recorded deviation); LUT (lut3d verified present) deferred until LUT assets exist as an upload kind. UI: 🎨 color panel (three sliders + reset), draft-drives-preview via a clipId-keyed grades map (NOT on segments — a slider tick must never respawn decoders), commits on release/keys/blur with a no-change skip. Review hardening: format=gbrp pins lutrgb to 8-bit (deep 10-bit sources negotiated 9..16-bit GBRP and crushed — reproduced); ColorDef decodes tolerantly (TS treats malformed color as neutral; strict Go bricked the whole log — scalar-field strictness gap recorded in ParseOps + hardening backlog); +0.5 rounds lutrgb like the canvas (real parity envelope ~1-2 LSB — golden-frame slice must budget it). Deviations: stills + <video> fallback ungraded, LUT deferred, paused stale-raw composite under another clip's grade is transient (tracked), Safari ctx.filter noop noted (Chrome-gated engine).
- **PR 37 — transitions** (design locked 2026-07-10): dissolve as the OUT-transition of a clip — `set_clip_transition` op {clip_id, transition?: {kind: "dissolve", duration}} (full replace, absent clears; duration clamped [0.1, 2]), `transition` on ClipInit, BOTH reducers + shared vector; bladeOps: left half keeps no transition, right half inherits it (the out edge moved). Semantics: the blend window is [cut, cut+D) — the OUTGOING clip continues D past its out point (source handles when they exist, tpad/decoder freeze when they don't — same freeze the preview already shows at EOF) while the incoming clip plays its head normally; timeline duration NEVER changes. Applies only when the next clip on the SAME track starts within epsilon of this clip's end; a dangling transition (gap/nothing follows) fades to black. Export: Flatten emits a BlendPiece with two inputs (left source seeked to its out point, right head) → `xfade=transition=fade:duration=D:offset=0` (xfade verified in this build), grades applied per input before the blend, captions/audio unaffected (audio crossfade is the ducking slice's territory; v1 audio keeps hard cuts — recorded deviation). Preview: EngineCanvas runs a SECOND session for the incoming segment during the window (the prebuffer infra already opens its decoder) and paints outgoing-then-incoming with globalAlpha = (t−cut)/D; paused seeks composite both one-shots. UI: a ⧓ badge at eligible boundaries (right-click/button on selected clip toggles dissolve, duration in a small popover).
- **PR 38 — audio ducking:** auto-duck music under speech; preview gain automation + export sidechain.
- **PR 39 — golden-frame check:** engine preview frame grabs vs export-extracted frames, perceptual diff in a `just` recipe (CI stays disabled per standing constraint).
- Depth-sequence render path: stays behind mock unless the R&D endpoint lands depth conditioning mid-milestone.
