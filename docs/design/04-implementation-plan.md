...
- **Next:** post-hoc lip-sync integration point (manifest-declared lipsync_post fallback), character voice binding (Character.voice_id → panel prefill), then M7. Backlog: transactional `ReorderShots(scene_id, ordered_ids)` RPC — the board renumbers client-side today (N sequential UpdateShot calls, non-atomic on partial failure, self-healing on the next reorder).
