...
- **Next:** audio refs in the generate panel (character voices), post-hoc lip-sync integration point (manifest-declared), then M6 exit: dialogue shot via ElevenLabs voice + Seedance audio-ref generation. Backlog: transactional `ReorderShots(scene_id, ordered_ids)` RPC — the board renumbers client-side today (N sequential UpdateShot calls, non-atomic on partial failure, self-healing on the next reorder).
