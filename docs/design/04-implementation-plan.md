...
- **Next:** elevenlabs + seedance adapters against recorded/mocked shapes (BYO-key vault + proxy path), audio refs in the panel, post-hoc lip-sync integration point. Backlog: transactional `ReorderShots(scene_id, ordered_ids)` RPC — the board renumbers client-side today (N sequential UpdateShot calls, non-atomic on partial failure, self-healing on the next reorder).
